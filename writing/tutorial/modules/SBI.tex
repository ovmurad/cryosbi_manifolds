...
\end{figure}	
	\begin{itemize}	
		\item Simulation-Based Inference (SBI): Bayesian inference on simulators on which we cannot evaluate a likelihood function $\theta \to p(x|\theta).$  	
...
\end{frame}
\begin{frame}
	\frametitle{SBI and Manifold Learning: Framework}
	For SBI, we focus on properties of the feature mapping $S_{\psi},$ via the geometry of its outputs on it's training data (the simulated data and labels), and the experimental data (the test data and it's predicted labels).
	We would like $S_{\psi}$ to have the following properties:
	\begin{itemize}
		\item Low Intrinsic Dimensionality

		\item Sufficiency for posterior prediction 

		\item Invariance to nuisance variation

		\item Disentanglement of relevant factors

		\item Coverage of and fidelity to experimental data
			\luke{Continue...}
	\end{itemize}
