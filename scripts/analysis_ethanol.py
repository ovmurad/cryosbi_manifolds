...
ALL_PARAMS = ("z")
SUBSAMPLE_KEY_PAIRS = (("points", "angles"),) + tuple(
    ("params", f"angles_{p}") for p in ALL_PARAMS
)
...
affs = final_dataset[f"affs|{DATA_NAME}-{DATA_NAME}"][:GRAD_ESTIM_SUBSAMPLE_SIZE]

#print(f"Running TSLasso for percentile={percentile} and cutoff={cutoff}.")

sample = np.arange(len(x_pts))
print(sample)
...
final_dataset["tslasso"][f"{DATA_NAME}"] = tslasso_results
...
pca_iter = local_weighted_pca_iter(
    x_pts=final_dataset[f"points|{DATA_NAME}"],
    weights=final_dataset[f"affs|{DATA_NAME}-{DATA_NAME}"][:EIGENGAP_ESTIM_SUBSAMPLE_SIZE],
    ncomp=16,
    bsize=256,
    in_place_norm=True,
    needs_norm=True,
